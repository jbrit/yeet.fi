...
export class Wallet {
   ...
   */
  startUp = async (accountChangeHook: (account: string) => unknown) => {
   ...
   */
  signIn = async () => {
    if (this.selector === undefined) return alert("call `startUp` first");
   ...
   */
  viewMethod = async ({ contractId, method, args = {} }: {contractId: string, method: string, args: {}}) => {
  ...
  callMethod = async ({
    ...
    return providers.getTransactionLastResult(outcome!); // TODO: handle `void`
  };
  ...
  getBalance = async (accountId: string) => {
    ...
    const amountString = utils.format.formatNearAmount(
      //@ts-ignore
